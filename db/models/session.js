...
async function getById(id) {
  try {
    ...
    if (!session) {
      console.log(`[Session Model] Session not found with ID: ${id}`);
      return null;
    }
    ...
    if (session.metadata) {
      try {
        ...
        if (typeof session.metadata === 'object' && session.metadata !== null) {
          console.log(`[Session Model] Session ${id} metadata is already an object, skipping parsing`);
          ...
          console.warn(`[Session Model] Session ${id} has metadata of unexpected type: ${typeof session.metadata}`);
          session.metadata = {};
        }
      } catch (e) {
        console.error(`[Session Model] Error parsing metadata for session ${id}:`, e);
    ...
    console.error(`[Session Model] Error retrieving session ${id}:`, error);
    return null;
  }
}
...
async function list(options = {}) {
  ...
  try {
    ...
    if (!Array.isArray(sessions)) {
      console.warn('[Session Model] Sessions query did not return an array');
      return [];
    }
    ...
    console.error('[Session Model] Error listing sessions:', error);
